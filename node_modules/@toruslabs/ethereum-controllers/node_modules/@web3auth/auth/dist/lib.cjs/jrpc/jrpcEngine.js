...
var errorClasses = require('./errors/error-classes.js');
var errorConstants = require('./errors/error-constants.js');
var utils = require('./errors/utils.js');
var safeEventEmitter = require('./safeEventEmitter.js');
var serializableError = require('./serializableError.js');
...
function constructFallbackError(error) {
  ...
  return {
    message: message || (error === null || error === void 0 ? void 0 : error.toString()) || utils.getMessageFromCode(codeNumber),
    code: codeNumber,
    stack,
 ...
 */
class JRPCEngine extends safeEventEmitter.SafeEventEmitter {
  constructor() {
    super();
    ...
    return new Promise(resolve => {
      const end = err => {
        ...
        if (error) {
          ...
          res.error = utils.serializeJrpcError(error, {
        ...
        if (res.error) {
          ...
          if (returnHandler) {
            if (typeof returnHandler !== "function") {
              end(new serializableError.SerializableError({
                code: -32603,
  ...
  static _checkForCompletion(_req, res, isComplete) {
    if (!("result" in res) && !("error" in res)) {
      throw new serializableError.SerializableError({
        code: -32603,
  ...
  async _handle(callerReq, cb) {
    if (!callerReq || Array.isArray(callerReq) || typeof callerReq !== "object") {
      const error = new serializableError.SerializableError({
        code: -32603,
...
function providerFromEngine(engine) {
  ...
  provider.sendAsync = async req => {
    ...
    if (res.error) {
      ...
      const err = utils.serializeJrpcError(res.error, {
        fallbackError: constructFallbackError(res.error),
        shouldIncludeStack: true
      });
      const errorCode = (_err$code = err === null || err === void 0 ? void 0 : err.code) !== null && _err$code !== void 0 ? _err$code : errorConstants.errorCodes.rpc.internal;
      const error = new errorClasses.JsonRpcError(errorCode, (_err$message = err === null || err === void 0 ? void 0 : err.message) !== null && _err$message !== void 0 ? _err$message : utils.getMessageFromCode(errorCode), err === null || err === void 0 ? void 0 : err.data);
      throw error;
    }
    return res.result;
  };
