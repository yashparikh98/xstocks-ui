...
var hdkey = require('ethereum-cryptography/hdkey');
...
function hashKeyWithIndex(key, index) {
  return new BN(hash.sha256().update(encUtils.hexToBuffer(encUtils.removeHexPrefix(key) + encUtils.sanitizeBytes(encUtils.numberToHex(index), 2))).digest("hex"), 16);
}
...
function getKeyPairFromPath(mnemonic, path) {
  ...
  const keySeed = Buffer.from(hdkey.HDKey.fromMasterSeed(seed).derive(path).privateKey).toString("hex");
  const starkEcOrder = signature.ec.n;
  return signature.ec.keyFromPrivate(grindKey(keySeed, starkEcOrder), "hex");
}
...
const StarkExEc = signature.ec.n;
