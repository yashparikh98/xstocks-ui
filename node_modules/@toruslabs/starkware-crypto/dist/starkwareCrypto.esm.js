...
const starkEc = new ec$1(new curves.PresetCurve({
  type: "short",
  prime: null,
  ...
  hash: hash.sha256,
  gRed: false,
  g: constantPoints$1[1]
}));
const constantPoints = constantPoints$1.map(coords => starkEc.curve.point(new BN(coords[0], 16), new BN(coords[1], 16)));
...
function getTransferMsgHash(amount, nonce, senderVaultId, token, receiverVaultId, receiverPublicKey, expirationTimestamp, condition = null) {
  assert(hasHexPrefix(token) && hasHexPrefix(receiverPublicKey) && (condition === null || hasHexPrefix(condition)), "Hex strings expected to be prefixed with 0x.");
...
function hashKeyWithIndex(key, index) {
  return new BN(hash.sha256().update(hexToBuffer(removeHexPrefix(key) + sanitizeBytes(numberToHex(index), 2))).digest("hex"), 16);
}
...
function getKeyPairFromPath(mnemonic, path) {
  ...
  const keySeed = Buffer.from(HDKey.fromMasterSeed(seed).derive(path).privateKey).toString("hex");
...
export { StarkExEc, ec, getAccountPath, getKeyPairFromPath, getLimitOrderMsgHash, getTransferMsgHash, grindKey, pedersen, sign, verify };
