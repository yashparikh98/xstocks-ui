...
var langrangeInterpolatePoly = require('./langrangeInterpolatePoly.js');
...
const GetPubKeyOrKeyAssign = async params => {
  ...
  const result = await some.Some(lookupPromises, async lookupResults => {
    ...
    if (keyResult && !nonceResult && !extendedVerifierId && !constants.LEGACY_NETWORKS_ROUTE_MAP[network]) {
      ...
      if (!nonceResult) {
        const metadataNonceResult = await metadataUtils.getOrSetSapphireMetadataNonce(network, keyResult.keys[0].pub_key_X, keyResult.keys[0].pub_key_Y);
...
const commitmentRequest = async params => {
  ...
  for (let i = 0; i < endpoints.length; i += 1) {
    ...
    const r = common.retryCommitment(p, 4);
    promiseArr.push(r);
  }
...
async function retrieveOrImportShare(params) {
  ...
  return some.Some(promiseArrRequest, async (shareResponseResult, sharedState) => {
    ...
    const oAuthPubKey = keyUtils.derivePubKey(ecCurve, oAuthKey);
    ...
    if (!nonceResult && !verifierParams.extended_verifier_id && !constants.LEGACY_NETWORKS_ROUTE_MAP[network]) {
      ...
      const metadataNonceResult = await metadataUtils.getOrSetSapphireMetadataNonce(network, thresholdPubKey.X, thresholdPubKey.Y, serverTimeOffset, oAuthKey);
      // rechecking nonceResult to avoid promise race condition.
    ...
    if (verifierParams.extended_verifier_id) {
      ...
      if (enableOneKey) {
        nonceResult = await metadataUtils.getOrSetNonce(legacyMetadataHost, ecCurve, serverTimeOffsetResponse, oAuthPubkeyX, oAuthPubkeyY, oAuthKey, !isNewKey);
        ...
        if (typeOfUser === "v2") {
          ...
          metadataNonce = await metadataUtils.getMetadata(legacyMetadataHost, {
    ...
    const oAuthKeyAddress = keyUtils.generateAddressFromPrivKey(keyType, oAuthKey);
    ...
    const finalWalletAddress = keyUtils.generateAddressFromPubKey(keyType, finalPubKey.getX(), finalPubKey.getY());
    let keyWithNonce = "";
    if (typeOfUser === "v1") {
      isUpgraded = null;
    ...
    if (keyType === constants.KEY_TYPE.SECP256K1) {
      ...
      if (keyWithNonce && !nonceResult.seed) {
        ...
        const decryptedSeed = await metadataUtils.decryptSeedData(nonceResult.seed, new BN(keyWithNonce, "hex"));
