...
var sha512 = require('ethereum-cryptography/sha512');
...
function toChecksumAddress(hexAddress) {
  ...
  const hash = Buffer.from(keccak.keccak256(buf)).toString("hex");
  let ret = "0x";
...
function getEd25519ExtendedPublicKey(keyBuffer) {
  const ed25519Curve = common.getKeyCurve(constants.KEY_TYPE.ED25519);
  const len = 32;
  ...
  const hashed = sha512.sha512(keyBuffer);
  if (hashed.length !== 64) {
...
function generateAddressFromEcKey(keyType, key) {
  if (keyType === constants.KEY_TYPE.SECP256K1) {
    ...
    const address = bs58.bs58.encode(publicKey);
    return address;
  }
...
const generateShares = async (ecCurve, keyType, serverTimeOffset, nodeIndexes, nodePubkeys, privKey) => {
  ...
  for (let i = 0; i < nodeIndexesBn.length; i++) {
    ...
    encPromises.push(eccrypto.encrypt(Buffer.from(nodePubKey.getPublic().encodeCompressed("hex"), "hex"), Buffer.from(shareJson.share.padStart(64, "0"), "hex")));
  }
...
exports.getEncryptionEC = getEncryptionEC;
