...
class SecurePubSub {
  constructor(options = {}) {
    ...
    this.options.serverUrl = options.serverUrl || constants.SESSION_SERVER_API_URL;
    this.options.socketUrl = options.socketUrl || constants.SESSION_SERVER_SOCKET_URL;
  ...
  async publish(topic, message) {
    ...
    const topicPrivKey = metadataHelpers.keccak256(Buffer.from(topic, "utf8"));
    const encryptedData = await metadataHelpers.encryptData(topicPrivKey.toString("hex"), message);
    const signature = await eccrypto.sign(topicPrivKey, metadataHelpers.keccak256(Buffer.from(encryptedData, "utf8")));
    const fetchBody = {
      key: eccrypto.getPublic(topicPrivKey).toString("hex"),
      // already padded
      data: encryptedData,
    ...
    return httpHelpers.post(`${this.options.serverUrl}/v2/channel/set`, fetchBody);
  }
  async subscribe(topic) {
    let isPromisePending = true;
    const topicPrivKey = metadataHelpers.keccak256(Buffer.from(topic, "utf8"));
    const topicPubKey = eccrypto.getPublic(topicPrivKey).toString("hex"); // already padded
    ...
    const returnPromise = new Promise((resolve, reject) => {
      const listener = async ev => {
        try {
          const decData = await metadataHelpers.decryptData(topicPrivKey.toString("hex"), ev);
  ...
  getSocketConnection() {
    ...
    const localSocketConnection = socket_ioClient.io(this.options.socketUrl, {
...
exports.SecurePubSub = SecurePubSub;
