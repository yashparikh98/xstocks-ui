...
var IChainInterface = require('./IChainInterface.js');

const INFURA_PROXY_URL = "https://api.web3auth.io/infura-service/v1";
const getDefaultNetworkId = chainNamespace => {
  if (chainNamespace === IChainInterface.CHAIN_NAMESPACES.EIP155) {
    return 1;
  } else if (chainNamespace === IChainInterface.CHAIN_NAMESPACES.SOLANA) {
    return 1;
  } else if (chainNamespace === IChainInterface.CHAIN_NAMESPACES.XRPL) {
    return 1;
  }
...
const getEvmChainConfig = (chainId, web3AuthClientId = "") => {
  const chainNamespace = IChainInterface.CHAIN_NAMESPACES.EIP155;
...
const getSolanaChainConfig = chainId => {
  const chainNamespace = IChainInterface.CHAIN_NAMESPACES.SOLANA;
...
const getXrplChainConfig = chainId => {
  const chainNamespace = IChainInterface.CHAIN_NAMESPACES.XRPL;
  if (chainId === 1) {
    return {
      chainNamespace,
      decimals: 15,
...
const getChainConfig = (chainNamespace, chainId, web3AuthClientId) => {
  if (chainNamespace === IChainInterface.CHAIN_NAMESPACES.OTHER) return null;
