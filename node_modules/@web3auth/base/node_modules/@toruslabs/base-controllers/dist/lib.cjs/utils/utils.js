...
function concatSig(v, r, s) {
  const rSig = util.fromSigned(r);
  const sSig = util.fromSigned(s);
  const vSig = util.bytesToBigInt(v);
  const rStr = padWithZeroes(Buffer.from(util.toUnsigned(rSig)).toString("hex"), 64);
  const sStr = padWithZeroes(Buffer.from(util.toUnsigned(sSig)).toString("hex"), 64);
  const vStr = util.stripHexPrefix(util.bigIntToHex(vSig));
  return util.addHexPrefix(rStr.concat(sStr, vStr));
}
...
const hashMessage = message => {
  ...
  const el = util.hashPersonalMessage(bufferedMessage);
  return Buffer.from(el);
};
const signMessage = async (privateKey, data) => {
  const privKey = Buffer.from(privateKey, "hex");
  const message = util.stripHexPrefix(data);
  const msgSig = util.ecsign(Buffer.from(message, "hex"), privKey);
  const rawMsgSig = concatSig(Buffer.from(util.bigIntToBytes(msgSig.v)), Buffer.from(msgSig.r), Buffer.from(msgSig.s));
  return rawMsgSig;
};
...
const handleRedirectParameters = (hash, queryParameters) => {
  ...
  if (!queryParameters.windowId) {
    if (Object.keys(hashParameters).length > 0 && hashParameters.state) {
      instanceParameters = JSON.parse(auth.safeatob(decodeURIComponent(decodeURIComponent(hashParameters.state)))) || {};
      error = hashParameters.error_description || hashParameters.error || error;
    } else if (Object.keys(queryParameters).length > 0 && queryParameters.state) {
      instanceParameters = JSON.parse(auth.safeatob(decodeURIComponent(decodeURIComponent(queryParameters.state)))) || {};
...
exports.addressSlicer = addressSlicer;
exports.broadcastChannelOptions = broadcastChannelOptions;
exports.concatSig = concatSig;
exports.formatDate = formatDate;
exports.formatSmallNumbers = formatSmallNumbers;
exports.formatTime = formatTime;
exports.getCustomDeviceInfo = getCustomDeviceInfo;
