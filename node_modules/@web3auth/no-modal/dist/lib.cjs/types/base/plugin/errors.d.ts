import { ErrorCodes, IWeb3AuthError, Web3AuthError } from "../errors";
export declare class WalletServicesPluginError extends Web3AuthError {
    protected static messages: ErrorCodes;
    constructor(code: number, message?: string, cause?: unknown);
    static fromCode(code: number, extraMessage?: string, cause?: unknown): IWeb3AuthError;
    static notInitialized(extraMessage?: string, cause?: unknown): IWeb3AuthError;
    static unsupportedConnector(extraMessage?: string, cause?: unknown): IWeb3AuthError;
    static providerRequired(extraMessage?: string, cause?: unknown): IWeb3AuthError;
    static web3authRequired(extraMessage?: string, cause?: unknown): IWeb3AuthError;
    static web3AuthNotConnected(extraMessage?: string, cause?: unknown): IWeb3AuthError;
    static alreadyInitialized(extraMessage?: string, cause?: unknown): IWeb3AuthError;
    static unsupportedChainNamespace(extraMessage?: string, cause?: unknown): IWeb3AuthError;
    static differentWeb3AuthNetwork(extraMessage?: string, cause?: unknown): IWeb3AuthError;
    static invalidParams(extraMessage?: string, cause?: unknown): IWeb3AuthError;
    static web3authNotInitialized(extraMessage?: string, cause?: unknown): IWeb3AuthError;
    static invalidSession(extraMessage?: string, cause?: unknown): IWeb3AuthError;
    static walletPluginNotConnected(extraMessage?: string, cause?: unknown): IWeb3AuthError;
}
