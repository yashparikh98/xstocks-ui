...
var StreamWindow = require('./StreamWindow.js');
 ...
 */
class PopupHandler extends BaseController.BaseController {
  constructor({
    config,
    state
  }) {
    ...
    this.defaultConfig = {
      features: utils.getPopupFeatures(enums.FEATURES_DEFAULT_POPUP_WINDOW),
      target: "_blank",
  ...
  async open() {
    ...
    if (!this.config.isWindowAlreadyOpen) {
      ...
      if (!localWindow) {
        ...
        const streamWindow = new StreamWindow.StreamWindow({
          config: {
            communicationEngine,
            communicationWindowManager,
    ...
    const localWindow = new StreamWindow.StreamWindow({
      config: {
        communicationEngine,
        communicationWindowManager,
        instanceId: this.config.instanceId,
